...
def draw_polygons( polygons, screen, color ):
    if len(polygons) < 3:
        print('Failed drawing; Add a polygon')
        return
    for point in range(0, len(polygons), 3):
        n = calculate_normal(polygons, point)
        v = [0, 0, 1]
        theta = dot_product(n, v)
        if theta > 0:
            draw_line(int(polygons[point][0]), int(polygons[point][1]), int(polygons[point+1][0]), int(polygons[point+1][1]), screen, color)
            draw_line(int(polygons[point][0]), int(polygons[point][1]), int(polygons[point+2][0]), int(polygons[point+2][1]), screen, color)
            draw_line(int(polygons[point+1][0]), int(polygons[point+1][1]), int(polygons[point+2][0]), int(polygons[point+2][1]), screen, color)
...
def add_sphere(polygons, cx, cy, cz, r, steps ):
    ...
    for lat in range(lat_start, lat_stop):
        for longt in range(longt_start, longt_stop+1):
            ...
            #i understand [index] and [(index + 1) % len(points)] as this will wrap around but why does [(index + steps + 1) % len(points)] work?????
